...
function avpt_init_elements(){
	jQuery(document).ready(function($){
		$("#archivespostaccordion .wea_content .item-posts").each(function(){  
			avpt_ikh_resize_elements($(this).parent().find(".panel-item").attr("id")); 
		});
	});
} 

function avpt_reload_current_elements(ipc_crid){
	jQuery(document).ready(function($){ 
		avpt_ikh_resize_elements(ipc_crid); 
...
function avpt_ikh_resize_elements(elementId) {
	jQuery(document).ready(function($){ 
			...
			if(ikh_config_elements["ikh_templates"]=="fit_to_sides") {
				avpt_ikh_fit_to_sides(ikh_config_elements,root_element);
			} else if(ikh_config_elements["ikh_templates"]=="fixed_width_left") {
				avpt_ikh_fixed_width_left(ikh_config_elements,root_element);
			} else if(ikh_config_elements["ikh_templates"]=="fixed_width_center") {
				avpt_ikh_fixed_width_center(ikh_config_elements,root_element);
			}
			
			$(root_element).find(".ik-post-load-more").show();
			$(root_element).find(".avpt-simple-paging").show(); 
	});	
}

function avpt_ikh_fit_to_sides(ikh_config_elements,root_element) {
	...
	jQuery(document).ready(function($){
		...
		if(ikh_main_width<=ikh_item_area_width) {
				...
				$(ikh_element).find(".avpt-simple-paging").removeAttr("style").css("position","relative");	 
		} else {
				...
				for( var ikhw = 60; ikhw > 0; ikhw-- ) {
					...
					if( ikh_main_width >= ikh_items_width ) {
						...
						for ( var ikh_j1=0; ikh_j1 < $(ikh_element).find(".ikh-post-item-box").length; ikh_j1++ ) {
							...
							var ikh_mrg_lft = 0; 
							if( ikh_j1 == 0 ) { 
								avpt_ikh_set_element_postion(ikh_element,ikh_j1,ikh_first_left,0,ikh_config_elements);
								ikh_mrg_lft = 0;
							} else {
								if( (ikh_j1%ikhw) == 0 ) {
									ikh_last_left = ikh_first_left;
								...
								if( ((ikh_j1-1)%(ikhw)) == 0 && ikhw>1 ) 
									ikh_last_left = ikh_last_left - ikh_margin_left-ikh_border_difference;
								avpt_ikh_set_element_postion(ikh_element,ikh_j1,ikh_last_left,(ikh_num_lines*(ikh_image_height1+ikh_margin_bottom)),ikh_config_elements);	
		...
		avpt_ikh_common_innerelement_load(ikh_element);  
	
	});
}	

function avpt_ikh_fixed_width_left(ikh_config_elements,root_element) {
...
function avpt_ikh_fixed_width_center(ikh_config_elements, root_element) {
	...
	jQuery(document).ready(function($){
		...
		for ( var ikh_j1=0; ikh_j1 < $(ikh_element).find(".ikh-post-item-box").length; ikh_j1++ ) {
			...
			var ikh_mrg_lft = 0; 
			if( ikh_j1 == 0 ) { 
				avpt_ikh_set_element_postion(ikh_element,ikh_j1,(ikh_first_left+ikh_remaining_width_left+((ikh_org_margin_left/2)/2)),0,ikh_config_elements);
				ikh_mrg_lft = 0;
				ikh_last_left = ikh_first_left+ikh_remaining_width_left+((ikh_org_margin_left/2)/2);
			} else {
				if( (ikh_j1%ikhw) == 0 ) {
...
function avpt_ikh_common_innerelement_load(ikh_element){
	jQuery(document).ready(function($){
		...
		} if($(ikh_element).find(" .avpt-simple-paging").length>0){
			main_final_height = main_final_height + ($(ikh_element).find(".avpt-simple-paging").height()+30);
			var left_bottom_pos = ($(ikh_element).find(" .avpt-simple-paging").width()/2); 
			$(ikh_element).find(" .avpt-simple-paging").css("display","none"); 
			$(ikh_element).find(" .avpt-simple-paging").css("top",pagination_top+"px").css("position","absolute");
			$(ikh_element).find(" .avpt-simple-paging").css("left",(($(ikh_element).width()/2)-left_bottom_pos)+"px");
			$(ikh_element).css("display","block"); 
			$(ikh_element).find(" .item-posts-wrap").css("height",main_final_height+"px");
			$(ikh_element).css("height",main_final_height+"px");
			$(ikh_element).find(" .avpt-simple-paging").css("position","absolute");
			$(ikh_element).find(" .avpt-simple-paging").css("display","block"); 
		} else {
		...
		var ikh_main_width = parseInt($(ikh_element).width());      
		if(ikh_main_width<=ikh_item_area_width) {
			$(ikh_element).find(" .avpt-simple-paging").css("position","relative").removeAttr("style").show();
			$(ikh_element).find(" .ik-post-load-more").css("position","relative").removeAttr("style").show();	
		    $(ikh_element).find(" .avpt-simple-paging").css("position","relative");
...
function avpt_ikh_set_element_postion(ikh_element,ikh_ele,ikh_left,ikh_top,ikh_config_elements_1){			 
...
function avpt_loadMorePosts(date_format,limit,elementId,total,request_obj){
	...
	jQuery(document).ready(function($){ 
			...
			request_obj.action = 'avpt_getMorePosts';
			request_obj.security = archivespostaccordion.avpt_security;
			request_obj.avpt_default_category = category_id;
...
function avpt_fillPosts(elementId,date_format,request_obj,flg_pr){
	...
	if(flg_v1==1) return;
 	jQuery(document).ready(function($){   
			...
			var avpt_default_category =  0;	
			if(flg_pr==2){ 
			    avpt_default_category =  $(root_element).find(".item-posts").find(".ik-drp-post-category").val(); 
				$(root_element).find(".ik-search-button").html($(".wp-load-icon").html()); 
			}
			else{ 
				avpt_default_category = request_obj.category_id; 
			...
			request_obj.action = 'avpt_getPosts';
			request_obj.date_format = date_format;
			request_obj.security = archivespostaccordion.avpt_security;
			request_obj.flg_pr = flg_pr; 
			request_obj.post_search_text = post_search_text; 
			request_obj.avpt_default_category = avpt_default_category;
...
function avpt_printData(elementId,data,flg){
	...
	jQuery(document).ready(function($){
		...
		avpt_reload_current_elements(elementId);
		flg_v1 = 0;	
	});	  
...
function avpt_init_accordion(){
	jQuery(document).ready(function($){ 
		...
		setInterval(function(){  
			 $("#archivespostaccordion .wea_content .pn-active").each(function(){  
				...
				$(ikh_element).find(".ikh-post-item").each(function(){
					 if( $(this).height() <= 50 ){
						avpt_ikh_resize_elements(attr_id);   
					 }
				});  
			});
		},500);
